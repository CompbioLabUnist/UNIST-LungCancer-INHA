"""
plot_signature_deconstructSigs_bar_subtype_clinical.py: Plot signature from deconstructSigs in bar graph with subtype & clinical separation
"""
import argparse
import itertools
import matplotlib
...
if __name__ == "__main__":
    ...
    for i, subtype in tqdm.tqdm(enumerate(order)):
        for j, clinical in enumerate(args.compare[1:]):
            ...
            for k, (column, color) in enumerate(zip(signatures, itertools.cycle(matplotlib.colors.XKCD_COLORS))):

...
if __name__ == "__main__":
    ...
    for i, chromosome in tqdm.tqdm(enumerate(chromosome_list)):
        ...
        for j in range(chromosome_data.shape[1]):
            axs[2][i].bar(x=j, height=len(list(filter(lambda x: chromosome_data.loc[x, j] <= 2 * (1 - args.threshold), primary_cancer_list))) / len(primary_cancer_list), width=1, align="edge", color="tab:blue", edgecolor="tab:blue", linewidth=None)
            axs[2][i].bar(x=j, height=len(list(filter(lambda x: chromosome_data.loc[x, j] <= 2 * (1 - args.threshold), precancer_list))) / len(precancer_list), width=1, align="edge", color="tab:cyan", edgecolor="tab:cyan", linewidth=None)

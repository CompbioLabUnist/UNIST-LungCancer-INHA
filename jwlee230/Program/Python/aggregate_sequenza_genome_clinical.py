...
if __name__ == "__main__":
    ...
    fig, axs = matplotlib.pyplot.subplots(nrows=6, ncols=len(chromosome_list), sharex="col", sharey="row", figsize=(len(chromosome_list) * 4, len(control_sample_list + case_sample_list) + 16), gridspec_kw={"height_ratios": [4, 4, len(control_sample_list), len(case_sample_list), 4, 4], "width_ratios": list(map(lambda x: x / step00.big, size_data.loc[chromosome_list, "length"]))})

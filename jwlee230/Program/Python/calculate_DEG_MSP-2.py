...
def correlation_stage(stage: str, MSP: str, gene: str) -> typing.Tuple[float, float, float, float, float, float]:
    ...
    regression = tuple(scipy.stats.linregress(tmp_data[MSP], tmp_data[gene]))
    spearman = tuple(scipy.stats.spearmanr(tmp_data[MSP], tmp_data[gene]))

    return regression[:2] + spearman + regression[-2:]

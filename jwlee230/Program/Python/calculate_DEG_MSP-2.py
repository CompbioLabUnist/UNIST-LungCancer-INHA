"""
calculate_DEG_MSP-2.py: calculate correlation between DEG and MSP with pearson correlation
"""
import argparse
import itertools
...
def correlation_stage(stage: str, sharing: str, gene: str) -> typing.Tuple[float, float, float, float, float]:
    ...
    regression = tuple(scipy.stats.linregress(tmp_data[sharing], tmp_data[gene]))
    spearman = tuple(scipy.stats.spearmanr(tmp_data[sharing], tmp_data[gene]))

    return regression[:2] + spearman + regression[-1:]

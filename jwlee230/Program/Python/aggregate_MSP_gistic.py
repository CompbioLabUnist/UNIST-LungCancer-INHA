"""
aggregate_MSP_gistic.py: Aggregate CNV results (Gistic) with MSP as genome view
"""
import argparse
import collections
...
if __name__ == "__main__":
    ...
    parser.add_argument("gistic", help="Get GISTIC data", type=str, nargs=4)
    ...
    low_gistic_data = pandas.read_csv(args.gistic[0], sep="\t")
    low_gistic_data = low_gistic_data.loc[(low_gistic_data["Unique Name"].str.contains("CN values"))]
    low_gistic_data["chromosome"] = list(map(lambda x: x[:x.find(":")], low_gistic_data["Wide Peak Limits"]))
    low_gistic_data["start"] = list(map(lambda x: int(x[x.find(":") + 1:x.find("-")]), low_gistic_data["Wide Peak Limits"]))
    low_gistic_data["end"] = list(map(lambda x: int(x[x.find("-") + 1:x.find("(")]), low_gistic_data["Wide Peak Limits"]))
    print(low_gistic_data)

    high_gistic_data = pandas.read_csv(args.gistic[1], sep="\t")
    high_gistic_data = high_gistic_data.loc[(high_gistic_data["Unique Name"].str.contains("CN values"))]
    high_gistic_data["chromosome"] = list(map(lambda x: x[:x.find(":")], high_gistic_data["Wide Peak Limits"]))
    high_gistic_data["start"] = list(map(lambda x: int(x[x.find(":") + 1:x.find("-")]), high_gistic_data["Wide Peak Limits"]))
    high_gistic_data["end"] = list(map(lambda x: int(x[x.find("-") + 1:x.find("(")]), high_gistic_data["Wide Peak Limits"]))
    print(high_gistic_data)
    ...
    for MSP in tqdm.tqdm(step00.sharing_columns[:2]):
        ...
        for chromosome in tqdm.tqdm(chromosome_list, leave=False):
            selected_low_data = low_gistic_data.loc[(low_gistic_data["chromosome"] == chromosome)]
            for index, row in selected_low_data.iterrows():
                if "Amplification Peak" in row["Unique Name"]:
                    axs[chromosome].scatter(-1, (row["start"] + row["end"]) // (2 * step00.big), s=400, c="tab:red", marker="^", edgecolor=None)
                elif "Deletion Peak" in row["Unique Name"]:
                    axs[chromosome].scatter(-1, (row["start"] + row["end"]) // (2 * step00.big), s=400, c="tab:blue", marker="v", edgecolor=None)

            selected_high_data = high_gistic_data.loc[(high_gistic_data["chromosome"] == chromosome)]
            for index, row in selected_high_data.iterrows():
                if "Amplification Peak" in row["Unique Name"]:
                    axs[chromosome].scatter(len(patient_list), (row["start"] + row["end"]) // (2 * step00.big), s=400, c="tab:red", marker="^", edgecolor=None)
                elif "Deletion Peak" in row["Unique Name"]:
                    axs[chromosome].scatter(len(patient_list), (row["start"] + row["end"]) // (2 * step00.big), s=400, c="tab:blue", marker="v", edgecolor=None)
    ...
    low_gistic_data = pandas.read_csv(args.gistic[2], sep="\t")
    low_gistic_data = low_gistic_data.loc[(low_gistic_data["Unique Name"].str.contains("CN values"))]
    low_gistic_data["chromosome"] = list(map(lambda x: x[:x.find(":")], low_gistic_data["Wide Peak Limits"]))
    low_gistic_data["start"] = list(map(lambda x: int(x[x.find(":") + 1:x.find("-")]), low_gistic_data["Wide Peak Limits"]))
    low_gistic_data["end"] = list(map(lambda x: int(x[x.find("-") + 1:x.find("(")]), low_gistic_data["Wide Peak Limits"]))
    ...
    high_gistic_data = pandas.read_csv(args.gistic[3], sep="\t")
    high_gistic_data = high_gistic_data.loc[(high_gistic_data["Unique Name"].str.contains("CN values"))]
    high_gistic_data["chromosome"] = list(map(lambda x: x[:x.find(":")], high_gistic_data["Wide Peak Limits"]))
    high_gistic_data["start"] = list(map(lambda x: int(x[x.find(":") + 1:x.find("-")]), high_gistic_data["Wide Peak Limits"]))
    high_gistic_data["end"] = list(map(lambda x: int(x[x.find("-") + 1:x.find("(")]), high_gistic_data["Wide Peak Limits"]))

...
def draw_violin(taxon: str) -> str:
    ...
    pairs = list()
    for a, b in itertools.combinations(stage_order, r=2):
        p = scipy.stats.mannwhitneyu(output_data.loc[(output_data["Subtype"] == a), taxon], output_data.loc[(output_data["Subtype"] == b), taxon])[1]
        if p < 0.05:
            pairs.append((a, b))
    ...
    if pairs:
        statannotations.Annotator.Annotator(ax, pairs, data=output_data, x="Subtype", y=taxon, order=stage_order, palette=step00.stage_color_code).configure(test="Mann-Whitney", text_format="simple", loc="inside", verbose=0, comparisons_correction=None).apply_and_annotate()

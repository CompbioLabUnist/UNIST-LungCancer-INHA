"""
get_data_composition_recurrence.py: get data composition which separated with recurrence condition
"""
import argparse
import itertools
import pandas
...
if __name__ == "__main__":
    ...
    output_data = pandas.DataFrame(columns=["Recurrence?", "Stage", "Number of Samples"], data=itertools.product(["Recurrence", "Non-recurrence"], step00.long_sample_type_list + ["Total"], [None])).set_index(keys=["Recurrence?", "Stage"], verify_integrity=True)
    print(output_data)

    for sample in tqdm.tqdm(args.input):
        raw_recurrence = clinical_data.loc[step00.get_patient(sample), "Recurrence"]
        if raw_recurrence == "YES":
            recurrence = "Recurrence"
        elif raw_recurrence == "NO":
        ...
        if output_data.loc[(recurrence, stage), "Number of Samples"] is None:
            output_data.loc[(recurrence, stage), "Number of Samples"] = 0
        output_data.loc[(recurrence, stage), "Number of Samples"] += 1

        if output_data.loc[(recurrence, "Total"), "Number of Samples"] is None:
            output_data.loc[(recurrence, "Total"), "Number of Samples"] = 0
        output_data.loc[(recurrence, "Total"), "Number of Samples"] += 1

"""
aggregate_sequenza_simple_clinical.py: Aggregate sequenza results as simple view with clinical data
"""
import argparse
...
if __name__ == "__main__":
    ...
    for i, chromosome in enumerate(chromosome_list):
        ...
        for j in tqdm.tqdm(range(chromosome_data.shape[1])):
            control_primary_proportion.append(len(list(filter(lambda x: chromosome_data.loc[x, j] >= 2 * (1 + args.threshold), control_primary_list))) / len(control_primary_list))
            control_precancer_proportion.append(len(list(filter(lambda x: chromosome_data.loc[x, j] >= 2 * (1 + args.threshold), control_precancer_list))) / len(control_precancer_list))
        ...
        axs[1][i].plot(range(chromosome_data.shape[1]), control_primary_proportion, color="navy", linestyle="--")
        axs[1][i].plot(range(chromosome_data.shape[1]), control_precancer_proportion, color="cyan", linestyle="--")
        axs[1][i].plot(range(chromosome_data.shape[1]), case_primary_proportion, color="navy", linestyle="-")
        axs[1][i].plot(range(chromosome_data.shape[1]), case_precancer_proportion, color="cyan", linestyle="-")

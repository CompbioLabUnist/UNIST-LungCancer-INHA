...
if __name__ == "__main__":
    ...
    parser.add_argument("--gene", help="Gene number to draw", type=int, default=20)
    ...
    categorical_data["gene1"] = arriba_data["gene1"]
    categorical_data["gene2"] = arriba_data["gene2"]
    categorical_data["value"] = list(map(lambda x: x.split("/")[0].title(), arriba_data["type"]))
    categorical_data["category"] = list(map(lambda x: "{0}+{1}".format(x[0], x[1]), zip(categorical_data["gene1"], categorical_data["gene2"])))
    categorical_data = categorical_data.loc[(categorical_data["gene1"].isin(common_genes)) | (categorical_data["gene2"].isin(common_genes))]

...
def get_quality(file_name: str) -> typing.Tuple[str, float, float]:
    """
    ...
    return (step00.get_id(file_name), mean, std)
...
if __name__ == "__main__":
    ...
    with multiprocessing.Pool(processes=args.cpus) as pool:
        quality_data = pandas.DataFrame(data=pool.map(get_quality, args.input), columns=["ID", "mean", "std"])
    print(quality_data)

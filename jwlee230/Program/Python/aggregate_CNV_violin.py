"""
aggregate_CNV_violin.py: Violin plot of CNV data for PRE-PRI comparing over chromosomes
"""
import argparse
import itertools
...
if __name__ == "__main__":
    ...
    for i, chromosome in tqdm.tqdm(list(enumerate(chromosome_list))):
        ...
        seaborn.violinplot(data=drawing_data, x="Stage", y=args.watching, order=drawing_stage_list, inner="box", palette=drawing_palette, cut=1, ax=axs[i // ncols][i % ncols])
        statannotations.Annotator.Annotator(axs[i // ncols][i % ncols], list(zip(drawing_stage_list, drawing_stage_list[1:])), data=drawing_data, x="Stage", y=args.watching, order=drawing_stage_list).configure(test="Mann-Whitney", text_format="simple", loc="inside", verbose=0).apply_and_annotate()

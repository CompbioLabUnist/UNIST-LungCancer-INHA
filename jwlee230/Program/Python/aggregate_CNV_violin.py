...
if __name__ == "__main__":
    ...
    for i, chromosome in tqdm.tqdm(list(enumerate(chromosome_list))):
        ...
        if pairs:
            statannotations.Annotator.Annotator(axs[i // ncols][i % ncols], pairs, data=drawing_data, x="Stage", y=args.watching, order=drawing_stage_list).configure(test="Mann-Whitney", text_format="simple", loc="inside", verbose=0, comparisons_correction=None).apply_and_annotate()

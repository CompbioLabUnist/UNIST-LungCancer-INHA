"""
aggregate_PureCN_violin_gene_clinical.py: Violin plot PureCN data for PRE-PRI comparing over genes by clincal data
"""
import argparse
import itertools
...
if __name__ == "__main__":
    ...
    for sample, gene in tqdm.tqdm(list(itertools.product(sample_list, gene_list))):
        ...
        tmp_data = input_data.loc[(input_data["ID"] == sample) & (input_data["chrom"] == chromosome) & (input_data["loc.start"] <= start) & (end <= input_data["loc.end"]), :]
        for index, row in tmp_data.iterrows():
            a.append(row[watching])
            weights.append(1)

        tmp_data = input_data.loc[(input_data["ID"] == sample) & (input_data["chrom"] == chromosome) & (start <= input_data["loc.start"]) & (input_data["loc.end"] <= end), :]
        for index, row in tmp_data.iterrows():
            a.append(row[watching])
            weights.append((row["loc.end"] - row["loc.start"] + 1) / length)

        tmp_data = input_data.loc[(input_data["ID"] == sample) & (input_data["chrom"] == chromosome) & (input_data["loc.start"] <= start) & (start <= input_data["loc.end"]), :]
        for index, row in tmp_data.iterrows():
            a.append(row[watching])
            weights.append((row["loc.end"] - start + 1) / length)

        tmp_data = input_data.loc[(input_data["ID"] == sample) & (input_data["chrom"] == chromosome) & (input_data["loc.start"] <= end) & (end <= input_data["loc.end"]), :]
        for index, row in tmp_data.iterrows():
            a.append(row[watching])
            weights.append((end - row["loc.start"] + 1) / length)
    ...
    for gene in tqdm.tqdm(gene_list):
        ...
        seaborn.violinplot(data=drawing_data, x="Stage", y=watching, order=drawing_stage_list, hue=args.compare[0], hue_order=args.compare[1:], inner="box", ax=ax)
        statannotations.Annotator.Annotator(ax, [((stage, args.compare[1]), (stage, args.compare[2])) for stage in drawing_stage_list], data=drawing_data, x="Stage", y=watching, order=drawing_stage_list, hue=args.compare[0], hue_order=args.compare[1:]).configure(test="Mann-Whitney", text_format="star", loc="inside", verbose=0).apply_and_annotate()

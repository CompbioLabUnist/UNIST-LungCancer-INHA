...
if __name__ == "__main__":
    ...
    for i, chromosome in enumerate(chromosome_list):
        ...
        for index, row in tqdm.tqdm(input_data.loc[(input_data["chromosome"] == chromosome)].iterrows()):
            chromosome_data.loc[row["Sample"], row["start"] // step00.big:row["end"] // step00.big] = 1.0 if ((1.0 - args.threshold) < row[args.watching] < (1 + args.threshold)) else row[args.watching]

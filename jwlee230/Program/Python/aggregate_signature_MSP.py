...
if __name__ == "__main__":
    ...
    for MSP in tqdm.tqdm(step00.sharing_columns[:2]):
        ...
        mosaic = [["MSP-bar"], ["Survival"], ["SBS-Precancer"], ["SBS-Primary"], ["DBS-Precancer"], ["DBS-Primary"]]
        fig, axs = matplotlib.pyplot.subplot_mosaic(mosaic=mosaic, figsize=(18 * 3, 18 * 2), gridspec_kw={"height_ratios": [1, 1, 2, 2, 2, 2]}, layout="tight")
        ...
        axs["SBS-Precancer"].set_ylabel("SBS in Precancer", fontsize="x-small")
        axs["SBS-Precancer"].grid(True)
        axs["SBS-Precancer"].legend(loc="upper right", fontsize="xx-small")

        for i, signature in tqdm.tqdm(list(enumerate(SBS_list)), leave=False):
            axs["SBS-Primary"].bar(x=range(len(primary_list)), height=SBS_data.loc[primary_list, :].iloc[:, i], bottom=SBS_data.loc[primary_list, :].iloc[:, :i].sum(axis=1), color=SBS_color_dict[signature], edgecolor=None, linewidth=0, label=signature)
        axs["SBS-Primary"].set_xticks(range(len(primary_list)), ["" for _ in patient_list], fontsize="xx-small")
        axs["SBS-Primary"].set_yticks([0, 2000, 4000, 6000], ["0", "2000", "4000", "6000"], fontsize="xx-small", rotation="vertical", verticalalignment="center")
        axs["SBS-Primary"].set_ylabel("SBS in Primary", fontsize="x-small")
        axs["SBS-Primary"].grid(True)
        axs["SBS-Primary"].legend(loc="upper right", fontsize="xx-small")
        ...
        axs["DBS-Precancer"].set_xticks(range(len(precancer_list)), ["" for _ in patient_list], fontsize="xx-small")
        axs["DBS-Precancer"].set_yticks([0, 50, 100], ["0", "50", "100"], fontsize="xx-small", rotation="vertical", verticalalignment="center")
        axs["DBS-Precancer"].set_ylabel("DBS in Precancer", fontsize="x-small")
        axs["DBS-Precancer"].grid(True)
        axs["DBS-Precancer"].legend(loc="upper right", fontsize="xx-small")

        for i, signature in tqdm.tqdm(list(enumerate(DBS_list)), leave=False):
            axs["DBS-Primary"].bar(x=range(len(primary_list)), height=DBS_data.loc[primary_list, :].iloc[:, i], bottom=DBS_data.loc[primary_list, :].iloc[:, :i].sum(axis=1), color=DBS_color_dict[signature], edgecolor=None, linewidth=0, label=signature)
        axs["DBS-Primary"].set_xticks(range(len(primary_list)), patient_list, fontsize="xx-small", rotation="vertical")
        axs["DBS-Primary"].set_yticks([0, 50, 100], ["0", "50", "100"], fontsize="xx-small", rotation="vertical", verticalalignment="center")
        axs["DBS-Primary"].set_ylabel("DBS in Primary", fontsize="x-small")
        axs["DBS-Primary"].grid(True)
        axs["DBS-Primary"].legend(loc="upper right", fontsize="xx-small")

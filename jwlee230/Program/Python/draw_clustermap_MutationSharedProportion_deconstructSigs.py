"""
draw_clustermap_MutationSharedProportion_deconstructSigs.py: draw Cluster Map with Mutation Shared Proportion vs. deconstructSigs
"""
import argparse
import itertools
import tarfile
...
if __name__ == "__main__":
    ...
    parser.add_argument("signature", help="Signature TSV file", type=str)
    ...
    if not args.signature.endswith(".tsv"):
        raise ValueError("SIGNATURE must end with .TSV!!")
    ...
    for stage, MSP in tqdm.tqdm(list(itertools.product(stage_list, step00.sharing_columns))):
        ...
        g = seaborn.clustermap(data=drawing_data, figsize=(len(signatures), len(drawing_data) / 4), row_cluster=True, col_cluster=True, cbar_pos=(-0.04, 0.2, 0.02, 0.6), row_colors=colors, tree_kws={"linewidths": 2.0}, xticklabels=True, yticklabels=False, square=False, cmap="Reds", vmin=0, vmax=1)
    ...
    for MSP in tqdm.tqdm(step00.sharing_columns):
        ...
        g = seaborn.clustermap(data=input_data[signatures], figsize=(len(signatures), len(input_data) / 4), row_cluster=True, col_cluster=True, cbar_pos=(-0.04, 0.2, 0.02, 0.6), row_colors=colors, tree_kws={"linewidths": 2.0}, xticklabels=True, yticklabels=False, square=False, cmap="Reds", vmin=0, vmax=1)

...
if __name__ == "__main__":
    ...
    figures = list()
    for MSP in tqdm.tqdm(step00.sharing_columns):
        tmp_color = pandas.DataFrame()
        tmp_color["Stage"] = color_data["Stage"]
        tmp_color["MSP"] = color_data[MSP]

        g = seaborn.clustermap(data=input_data, figsize=(32, 18), row_cluster=True, col_cluster=True, cbar_pos=(-0.04, 0.2, 0.02, 0.6), col_colors=tmp_color, xticklabels=False, yticklabels=False, square=False, z_score=0, cmap="coolwarm", center=0, robust=True)
    ...
    for stage, MSP in tqdm.tqdm(list(itertools.product(stage_list, step00.sharing_columns))):
        ...
        tmp_color = pandas.DataFrame()
        tmp_color["MSP"] = color_data.loc[selected_samples, MSP]

        g = seaborn.clustermap(data=selected_data, figsize=(32, 18), row_cluster=True, col_cluster=True, cbar_pos=(-0.04, 0.2, 0.02, 0.6), col_colors=tmp_color, xticklabels=False, yticklabels=False, square=False, z_score=0, cmap="coolwarm", center=0, robust=True)

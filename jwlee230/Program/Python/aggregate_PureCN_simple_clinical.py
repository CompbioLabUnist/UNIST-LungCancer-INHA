"""
aggregate_PureCN_simple_clinical.py: Aggregate PureCN results as simple view with clinical data
"""
import argparse
...
if __name__ == "__main__":
    ...
    size_data = pandas.read_csv(args.size, sep="\t", header=None, names=["chrom", "length"]).set_index(keys="chrom", verify_integrity=True)
    print(size_data)
    ...
    fig, axs = matplotlib.pyplot.subplots(nrows=4, ncols=len(chromosome_list), sharex="col", sharey="row", figsize=(len(chromosome_list) * 4, 32), gridspec_kw={"height_ratios": [8, 8, 8, 8], "width_ratios": list(map(lambda x: x / step00.big, size_data.loc[chromosome_list, "length"]))})

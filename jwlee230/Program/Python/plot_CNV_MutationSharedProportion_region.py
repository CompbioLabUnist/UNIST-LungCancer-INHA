...
if __name__ == "__main__":
    ...
    for stage, MSP in itertools.product(stage_list, step00.sharing_columns):
        ...
        print(f"min. {stage} & {MSP}", list(tmp_data.loc[(tmp_data["Region"] == min(tmp_data["Region"]))].sort_values(MSP).iloc[:3, 1]))
        print(f"max. {stage} & {MSP}", list(tmp_data.sort_values("Region", ascending=False).iloc[:3, 1]))
        del output_data[MSP]

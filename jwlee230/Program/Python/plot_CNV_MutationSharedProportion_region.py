...
if __name__ == "__main__":
    ...
    for stage, MSP in itertools.product(stage_list, step00.sharing_columns):
        ...
        print(f"min. {stage} & {MSP}", tmp_data.loc[(tmp_data["Region"] == min(tmp_data["Region"]))].sort_values(MSP).iloc[0, 1])
        print(f"max. {stage} & {MSP}", tmp_data.loc[(tmp_data["Region"] == max(tmp_data["Region"]))].sort_values(MSP, ascending=False).iloc[0, 1])
        del output_data[MSP]
    ...
    for MSP in tqdm.tqdm(step00.sharing_columns):
        ...
        for (x1, s1), (x2, s2) in [(("Lower", stage), ("Higher", stage)) for stage in stage_list] + [((x, a), (x, b)) for x in MSP_order for a, b in itertools.combinations(stage_list, r=2)]:
            stat, p = scipy.stats.mannwhitneyu(output_data.loc[(output_data[MSP] == x1) & (output_data["Stage"] == s1), "Region"], output_data.loc[(output_data[MSP] == x2) & (output_data["Stage"] == s2), "Region"])
            if p < 0.05:
                compare_list.append(((x1, s1), (x2, s2)))

        try:
            stat, p = scipy.stats.kruskal(*[output_data.loc[(output_data[MSP] == x) & (output_data["Stage"] == s), "Region"] for x, s in itertools.product(MSP_order, stage_list)])
        except ValueError:
            p = 1.0
        ...
        compare_list = list()
        for (x1, s1), (x2, s2) in [(("Lower", stage), ("Higher", stage)) for stage in stage_list] + [((x, a), (x, b)) for x in MSP_order for a, b in itertools.combinations(stage_list, r=2)]:
            stat, p = scipy.stats.mannwhitneyu(output_data.loc[(output_data[MSP] == x1) & (output_data["Stage"] == s1), "Region-Loss"], output_data.loc[(output_data[MSP] == x2) & (output_data["Stage"] == s2), "Region-Loss"])
            if p < 0.05:
                compare_list.append(((x1, s1), (x2, s2)))

        try:
            stat, p_loss = scipy.stats.kruskal(*[output_data.loc[(output_data[MSP] == x) & (output_data["Stage"] == s), "Region-Loss"] for x, s in itertools.product(MSP_order, stage_list)])
        except ValueError:
            p_loss = 1.0
        ...
        compare_list = list()
        for (x1, s1), (x2, s2) in [(("Lower", stage), ("Higher", stage)) for stage in stage_list] + [((x, a), (x, b)) for x in MSP_order for a, b in itertools.combinations(stage_list, r=2)]:
            stat, p = scipy.stats.mannwhitneyu(output_data.loc[(output_data[MSP] == x1) & (output_data["Stage"] == s1), "Region-Gain"], output_data.loc[(output_data[MSP] == x2) & (output_data["Stage"] == s2), "Region-Gain"])
            if p < 0.05:
                compare_list.append(((x1, s1), (x2, s2)))

        try:
            stat, p_gain = scipy.stats.kruskal(*[output_data.loc[(output_data[MSP] == x) & (output_data["Stage"] == s), "Region-Gain"] for x, s in itertools.product(MSP_order, stage_list)])
        except ValueError:
            p_gain = 1.0

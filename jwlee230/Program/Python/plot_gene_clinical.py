...
if __name__ == "__main__":
    ...
    control_samples = list(map(step00.get_id, sorted(filter(lambda x: step00.get_patient(x.split("/")[-1].split(".")[0]) in control_patients, args.input))))
    ...
    evaluation_data = pandas.DataFrame(data=numpy.zeros((len(gene_list), 4)), index=gene_list, columns=["Fisher", "Chi2", "Barnard", "Boschloo"], dtype=float)
    with multiprocessing.Pool(args.cpus) as pool:
        for derivation in tqdm.tqdm(evaluation_data.columns):
            evaluation_data.loc[:, derivation] = pool.starmap(query_heatmap, [(gene, derivation) for gene in list(evaluation_data.index)])

"""
compare_VDT_signature.py: compare VDT (volume doubling time) vs. mutational signature
"""
import argparse
import itertools
import tarfile
...
if __name__ == "__main__":
    ...
    for stage, signature in tqdm.tqdm(list(itertools.product(step00.long_sample_type_list, signature_list))):
        tmp_data = output_data.loc[(output_data["Stage"] == stage), ["Volume_Doubling_Time", signature]]
        if tmp_data.shape[0] < 3:
            continue

        r, p = scipy.stats.pearsonr(tmp_data[signature], tmp_data["Volume_Doubling_Time"])

        fig, ax = matplotlib.pyplot.subplots(figsize=(24, 24))

        g = seaborn.jointplot(data=tmp_data, x=signature, y="Volume_Doubling_Time", kind="reg", height=24, ratio=6, color=step00.stage_color_code[stage])
    ...
    for signature in tqdm.tqdm(signature_list):
        tmp_data = output_data.loc[:, ["Volume_Doubling_Time", signature]]
        ...
        g = seaborn.jointplot(data=tmp_data, x=signature, y="Volume_Doubling_Time", kind="reg", height=24, ratio=6)
        g.fig.text(0.5, 0.75, "r={0:.3f}, p={1:.3f}".format(r, p), color="k", fontsize="small", horizontalalignment="center", verticalalignment="center", bbox={"alpha": 0.3, "color": "white"}, fontfamily="monospace")

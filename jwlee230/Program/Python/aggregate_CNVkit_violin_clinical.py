"""
aggregate_CNVkit_violin_clinical.py: Aggregate CNVkit results as violin view with clinical data
"""
import argparse
import itertools
...
if __name__ == "__main__":
    ...
    for i, chromosome in tqdm.tqdm(list(enumerate(chromosome_list))):
        ...
        seaborn.violinplot(data=drawing_data, x="Stage", y=watching, order=drawing_stage_list, inner="box", palette=drawing_palette, hue=args.compare[0], hue_order=args.compare[1:], cut=1, ax=axs[i // ncols][i % ncols])
        statannotations.Annotator.Annotator(axs[i // ncols][i % ncols], list(((stage, args.compare[1]), (stage, args.compare[2])) for stage in drawing_stage_list), data=drawing_data, x="Stage", y=watching, order=drawing_stage_list, hue=args.compare[0], hue_order=args.compare[1:]).configure(test="Mann-Whitney", text_format="simple", loc="inside", verbose=0).apply_and_annotate()

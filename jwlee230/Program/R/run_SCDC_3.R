...
main <- function(input_file, annotation_file, reference_file, output_file)
{
    ...
    reference_data <- as.data.frame(as.matrix(readRDS(reference_file)))
    reference_data <- reference_data[, barcodes]

    reference_eset <- ExpressionSet(as.matrix(reference_data), phenoData=AnnotatedDataFrame(data.frame(row.names=colnames(reference_data), samples=annotation_data[["Sample"]], clusters=annotation_data[["Sub_Cluster"]])))
    print(reference_eset)

    estimation_proportion <- SCDC_prop(bulk.eset=input_eset, sc.eset=reference_eset, ct.varname="clusters", ct.sub=annotation_data[["Sub_Cluster"]], sample="samples", verbose=TRUE, iter.max=10000)
